...
import ar.edu.itba.it.paw.services.services.NewAvertisementService;
import ar.edu.itba.it.paw.web.SessionUserManager;
import ar.edu.itba.it.paw.web.UserManager;

@SuppressWarnings("serial")
public class NewAdvertisement extends HttpServlet {
	...
	protected void doGet(HttpServletRequest req, HttpServletResponse resp)
		...
		req.getRequestDispatcher(
				resp.encodeURL("/WEB-INF/jsp/newAdvertisement.jsp")).forward(
				req, resp);
	}
	...
	protected void doPost(HttpServletRequest req, HttpServletResponse resp)
		...
		NewAdvertisementInterf na = new NewAvertisementService();
		...
		try {
			...
			if (errorManager.hasErrors()) {
				...
				req.getRequestDispatcher(
						resp.encodeURL("/WEB-INF/jsp/newAdvertisement.jsp"))
						.forward(req, resp);
				return;
			}
			...
			na.addAdvertisement(property, manager.getUser());
	...
	private void setParameters(Map<String, String> info,
		...
		for (String key : set) {
			if (key.equals("cancha de paddle")) {
				req.setAttribute("canchaDePaddle", info.get(key));
			} else
